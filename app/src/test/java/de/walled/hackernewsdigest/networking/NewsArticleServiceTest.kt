...
class NewsArticleServiceTest {
    ...
    fun `Given a call returns successfully then the result will be a SingleArticle with data`() {
        val expectedItem = HackerNewsItem(42)
    ...
    private fun createTestAPI(resultFunction: () -> Single<Response<HackerNewsItem>>): NewsArticleApi {
        return object : NewsArticleApi {
            override fun loadArticle(id: Long): Single<Response<HackerNewsItem>> {
    ...
    companion object {
        fun createSuccesfulResponseWithData(data: HackerNewsItem): Single<Response<HackerNewsItem>> {
            return Observable.create<Response<HackerNewsItem>> { }.single(Response.success(data))
        }

        fun createSuccesfulResponseWithoutData(): Single<Response<HackerNewsItem>> {
            return Observable.create<Response<HackerNewsItem>> { }.single(Response.success(null))
        }

        fun createResourceNotFoundResponse(): Single<Response<HackerNewsItem>> {
            val errorBody = ResponseBody.create(MediaType.parse("text/plaintext"), "")
            val error = Response.error<HackerNewsItem>(404, errorBody)
            return Observable.create<Response<HackerNewsItem>> { }.single(error)
        }

        fun createServiceUnavailableResponse(): Single<Response<HackerNewsItem>> {
            val errorBody = ResponseBody.create(MediaType.parse("text/plaintext"), "")
            val error = Response.error<HackerNewsItem>(503, errorBody)
            return Observable.create<Response<HackerNewsItem>> { }.single(error)
        }

        fun createUnknownErrorResponse(): Single<Response<HackerNewsItem>> {
            val errorBody = ResponseBody.create(MediaType.parse("text/plaintext"), expectedUnknownErrorText)
            val error = Response.error<HackerNewsItem>(999, errorBody)
            return Observable.create<Response<HackerNewsItem>> { }.single(error)
        }
